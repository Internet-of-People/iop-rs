...
pub trait SyncDidVault {
    ...
    fn set_active(&mut self, did: &Did) -> Result<()>;

    fn record_by_auth(&self, auth: &Authentication) -> Result<HdRecord>;
    // fn restore_id(&mut self, did: &Did) -> Result<()>;
    // fn signer_by_auth(&self, auth: &Authentication) -> Result<Box<dyn SyncSigner>>;
    fn signer_by_auth(&self, auth: &Authentication) -> Result<PrivateKeySigner>;

    fn create(&mut self, label: Option<Label>) -> Result<HdRecord>;
    fn update(&mut self, record: HdRecord) -> Result<()>;
}
...
impl InMemoryDidVault {
    ...
    pub fn morpheus_xsk(&self) -> Result<EdExtPrivateKey> {
    ...
    fn public_key(&self, idx: i32) -> Result<MPublicKey> {
        let did_xsk = self.morpheus_xsk()?.derive_hardened_child(idx)?;
        let key = did_xsk.neuter().public_key();
        Ok(key.into())
    }

    fn private_key(&self, idx: i32) -> Result<MPrivateKey> {
        let did_xsk = self.morpheus_xsk()?.derive_hardened_child(idx)?;
    ...
    fn did_by_label(&self, label: &Label) -> Result<Did> {
        self.records
            .iter()
            .filter_map(|rec| if rec.label == *label { Some(rec.did()) } else { None })
            .next()
            .with_context(|| format!("Label {} is not found in vault", label))
    }
...
impl SyncDidVault for InMemoryDidVault {
    ...
    fn set_active(&mut self, did: &Did) -> Result<()> {
        let idx = self
            .index_of_did(did)
            .with_context(|| format!("Vault does not contain DID {}", did))?;
    ...
    fn record_by_auth(&self, auth: &Authentication) -> Result<HdRecord> {
        let rec_opt = self
            ...
            .cloned()
            .next();
        rec_opt.with_context(|| format!("Vault does not contain {:?}", auth))
    }
    ...
    fn create(&mut self, label_opt: Option<Label>) -> Result<HdRecord> {
    ...
    fn update(&mut self, record: HdRecord) -> Result<()> {
        ...
        let old_rec =
            self.records.get(idx).ok_or_else(|| anyhow!("Index {} is invalid in record", idx))?;
...
mod tests {
    ...
    fn test_vault<T: SyncDidVault>(vault: &mut T) -> Result<()> {
