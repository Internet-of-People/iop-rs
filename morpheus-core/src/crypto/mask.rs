...
pub fn collapse_json_subtree(data: &serde_json::Value) -> serde_json::Result<serde_json::Value> {
    match data {
        serde_json::Value::Array(arr) => {
            let mut canonical_json_items = Vec::new();
            for item in arr {
                let masked_item = collapse_json_subtree(item)?;
        ...
        serde_json::Value::Object(obj) => {
            ...
            for key in keys {
                let value = obj.get(key).expect("serde_json keys() impl error");
                let canonical_key = canonical_json(&serde_json::Value::String(key.to_owned()))?;
                let masked_val = collapse_json_subtree(value)?;
...
pub fn json_digest<T: serde::Serialize>(data: &T) -> Fallible<String> {
    ...
    let digest_json = match &json_value {
        serde_json::Value::Object(_obj) => collapse_json_subtree(&json_value),
        serde_json::Value::Array(_arr) => collapse_json_subtree(&json_value),
